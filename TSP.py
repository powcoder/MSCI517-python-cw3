...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 8 09:01:27 2019
...
def TotalDistance(sol, dist, n):
    ...
    totalDist = totalDist + dist[sol[n-1]][sol[0]]
    return totalDist
...
df_tsp = pd.read_csv('I1.csv', index_col='ID')
n = len(df_tsp.index)

# Here (for now) we create a dist list containing n lists, each of n items, all set to 0
...
for i in range(n):
    for j in range(n):
        dist[i][j] = ((df_tsp.iloc[i,0] - df_tsp.iloc[j,0]) ** 2 + (df_tsp.iloc[i,1] - df_tsp.iloc[j,1]) ** 2) ** 0.5
